...
describe('usePagination', () => {
  ...
  it('应该支持自定义 pageSizes', () => {
    ...
    const { pageSizes } = usePagination(1, customSizes);
    expect(pageSizes.value).toEqual(customSizes);
  });
  ...
  it('应该能动态更新 pageSizes', () => {
    ...
    expect(pageSizes.value).toEqual(newSizes);
  });
  ...
  it('应该支持同时配置 pageNumber、pageSizes 和 pageSize', () => {
    const { pagination, pageSizes, resetPagination } = usePagination(2, [25, 50, 100], 50);
    expect(pagination.value.pageNumber).toBe(2);
    expect(pagination.value.pageSize).toBe(50);
    expect(pageSizes.value).toEqual([25, 50, 100]);
